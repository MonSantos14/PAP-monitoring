...
<body class="ms-fill">
    ...
    <div class="d-flex wrapper" id="wrapper">
        ...
        <div id="page-content-wrapper">
          ...
          <div class="container-fluid">
            <!-- Page Title -->
            <h1 class="page-heading mt-2"><ion-icon name="document-outline" size="large"></ion-icon> Drafts</h1>
            ...
            <div class="container-fluid py-2">
              <div class="card table-container">
                <div class="card-header">
                  <h3 class="table-heading">List of Proposal Drafts</h3>
                </div>
                <div class="card-body">
                  ...
                  <tbody>
                      @if ($drafts->count())
                        @foreach ($drafts as $draft)
                            <tr class="unread clickable-row" data-href="{{ route('draft-proposal', $draft->id) }}">
                            
                                <td class="tblink">{{ $draft->proposal_refID }}</td>
                                <td class="tblink">{{ $draft->proposal_title }}</td>
                                <td>{{ $draft->proposal_leader }}</td>
                                <td>{{ $draft->proposal_duration }} Month/s</td>
                                <td>{{ $draft->proposal_location }}</td>
                                <td><span class="badge status-draft">{{ $draft->proposal_status }}</span></td>
                                <td>{{ date('m-s-y', strtotime($draft->created_at)) }}</td>
                            </tr>
                        @endforeach
                      @else
